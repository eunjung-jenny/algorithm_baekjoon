...
bulb_n = int(input())
bulb_state = list(map(int, input().split()))
stu_n = int(input())
...
for _ in range(stu_n):
    ...
    if gender == 1:
        for i in range(1, bulb_n//num+1):
            bulb_state[num*i-1] ^= 1

    else:
        bulb_state[num-1] ^= 1
        for i in range(1, min(num-1, bulb_n-num)+1):
            if bulb_state[num+i-1] == bulb_state[num-i-1]:
                bulb_state[num+i-1] ^= 1
                bulb_state[num-i-1] ^= 1
            else:
                break

for i in range(1, len(bulb_state)+1):
    print(bulb_state[i-1], end = ' ')
    if i % 20 == 0: 
        print()
